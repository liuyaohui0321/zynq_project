...
u8_ring_buffer_t  ring_buffer;
#define RINGBUFF_SIZE      512
u8 data_ringbuff[RINGBUFF_SIZE] = {0};
//// 中断处理函数
...
void UartInterruptHandler(void *CallBackRef)
{
    ...
    if (IsrStatus & (XUARTPS_IXR_RXOVR| XUARTPS_IXR_RXFULL | XUARTPS_IXR_TOUT))
    {
		...
		u8_ring_buffer_queue_arr(&ring_buffer,ReceiveBuffer,ReceiveCount);
...
int UartPsIntr(void)
{
    ...
    /* 初始化ringbuffer */
    u8_ring_buffer_init(&ring_buffer, (char *)data_ringbuff, RINGBUFF_SIZE);
